...
So, insted of sticky ourself in struggling with code sharing we moved on and began to **analyze where were we wasing time** among the company.

> The most time expensive moment in every developer's day is called **_WTF_**  
> <small>_(Wath The Fuck)_</small>  
> 
...
* to write a complex solution is way more complex than to write many small little ones that can work together
* to write my own DOM library is way more time consuming compared to use a jQuery custom build
* to write my own knowledge base abour Javascript is crazy, I can search, read and link valuable articles that are already available online
...
<small>(and I am one of them!)</small>

## Post Fact

When it comes to practical coding we have a simple action point which is **micro coding**.

We left the idea of complete reusable _Features_ way behind us. Today we focus in **writing the less possible amount of code** for each single module of our applications.
...
At the beginning they used a simple [DropBox shared folder](dbox) to put their micro libraries, today we evolved a little bit when we introduced out second action point.

Our second practical action point is **Open Source First**.

> If the solutions exists among the Open Source community we use it,  
> if we create our own solution, we publish it back for other to use it.

Today we use [CommonJS](commonjs) and [NPM](npm) to write our Javascript, if we need a library we search _NPM_ or _GitHub_, if it exists we use it, if it exists on _GitHub_ but it is not available through _NPM_ we contribute to the project to fix the issue, then we use through _NPM_.

This behaviour is much more than a merely _giving back to the community_.  

> By actively publish open source code **we get free help from the community** that contribute to
> our repositories by doing bug-traking, bug-fixing and code enhancement.
...
> There is nothing more than money to light up your willing of doing.
...
The prize was always quite impressive and at the very beginning some managers had an heart attack

We have a play-room so we created a _ChallengeME_ app to invite a colleague to the table tennis, we do afterworks so we suggested a _PubRating_ app internal to the company... and much more!
...
> all around so **he/she coach other colleagues to improve**. For free!

## Conclusions

Today my company is a better place to work, the quality have increased so much and eventually we start to have some really big thing around code sharing and reusability.

We are also planning to move more into a product business model, we can do that today because we have the culture of enterprise coding and reusability built into every of our developers.
...
This is good, he will learn even more from them but he can not stop to hang out with his old mates, they have become friends through this experience, so **all the knowledge is coming back to us**!

We started this experience more than one year ago, we faced the problem, we spiked some dead ends but eventually we made it. 

What we learn is that **code sharing and reusability is not about tools but about culture**, is about people and knowledge, is about sharing the objective to spare some time from our daily struggles and use it in a more fun and proficient way.
